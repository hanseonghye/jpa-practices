...
1. __게시판 게시물(Board)의 댓글(Comment)__
    1) 게시판 게시물 보기 페이지에서 게시물 내용 아래에 달린 댓글들이다. Board View 페이지에서 다음과 같은 관계가 있을 것이다  
        Board -> Comment Navigational Association  
    2) 고려해야 할 반대 방향의 비즈니스 로직은 없어 보인다
...
3. OneToMany
    1) 참조 필드의 타입으로 Map, Set, List, Collection등을 사용하게 된다.
    2) JPA 2.0 부터 지원
...
4. __JpaBoardRepositoryTest.java__
    ...
    4) test04BoardListLazyProblemSolved()
        ...
        + 해결을 위한 방법은 JPQL(QueryDSL)로 해결할 수 있다. 
            - QueryDSL 경우에는 innerJoin(), fetchJoin()를 사용해 개선할 수 있다(jpa03-model02 참고)
            - 게시판 리스트에서는 User 정보가 다 필요없다. 이름과 번호 정도일 것이다. Projection과 DTO를 함께 고려하면 성능 향상에 더 좋을 수 있다.
            - 문제는 Projection을 하면 Join Fetch를 할 수 없다는 점이다.
            - 이유는 Fetch Join은 기본적으로 select에 1개 이상의 Entity가 올 수 없다.(Fetch Join 뒤의 Entity에 별칭을 줄 수 없는 이유이기도 하다)
            - 하지만, QueryDSL에서는 innerJoin() 만으로도 Fetch Join으로 생성된 쿼리와 같은 쿼리 실행이 가능하다.(jpa03-model02 참고)
        + 테스트 실행과 쿼리 로그 
